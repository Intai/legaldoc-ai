...
def clear_clickhouse(settings):
    """Truncate all SigNoz tables in ClickHouse.

    Silently skips when ClickHouse is unreachable (SigNoz stack not running).
    """
    try:
        ...
        for db in SIGNOZ_DATABASES:
            rows = client.query(f"SHOW TABLES FROM {db}")
            for (table,) in rows.result_rows:
                client.command(f"TRUNCATE TABLE {db}.{table}")
                truncated += 1
        print(f"Truncated {truncated} ClickHouse tables.")
    except Exception:
        print("ClickHouse not available — skipping telemetry cleanup.")
...
async def seed():
    ...
    # Truncate telemetry last so traces generated by the seed itself are cleared.
